...
| **Thành viên 1** (Leader / SOA Backend Architect) | **Trương Thế Hải Thịnh (Chan)** – 23725051 |
| --- | --- |
| **Thành viên 2** (Data Engineer) | Nguyễn Văn A – *[MSSV]* |
| **Thành viên 3** (ML / Cloud Engineer) | Trần Thị B – *[MSSV]* |
| **Thành viên 4** (Frontend Developer) | Lê Hoàng D – *[MSSV]* |
| **Thành viên 5** (QA / Data Analyst) | Phạm Minh E – *[MSSV]* |
| **Git Repository** | [https://github.com/TruongTheHaiThinh/SmartGPA-Academic-Analytics-Platform](https://www.google.com/search?q=https://github.com/TruongTheHaiThinh/SmartGPA-Academic-Analytics-Platform) |

### Cấu trúc nhánh Git

| Nhánh | Mục đích | Người phụ trách |
| --- | --- | --- |
| `feature/auth-gateway` | Module 1 – Xác thực, phân quyền JWT (4 roles) & API Gateway | Chan |
| `feature/databricks-pipeline` | Module 2 – Hạ tầng Delta Lake, Pipeline ETL nạp điểm thô | Thành viên 2 |
| `feature/simulation-engine` | Module 3 – Logic core tính điểm đảo (LT/TH/Tích hợp) | Chan |
| `feature/ml-prediction` | Module 4 – MLflow Model dự báo rớt môn (Điểm F) | Thành viên 3 |
| `feature/client-portal` | Module 5 – Giao diện Web UI tích hợp thanh trượt giả lập | Thành viên 4 |
| `feature/analytics-dashboard` | Module 6 – Kiểm thử Pytest & Databricks SQL Dashboard | Thành viên 5 |
| `develop` | Tích hợp tất cả các feature branch sau khi được Leader review | Cả nhóm |
| `main` | **Push cuối cùng – Bản hoàn chỉnh để nộp và deploy** | Cả nhóm |

> **Quy trình phối hợp làm việc:**
> 1. Mỗi thành viên làm việc trên nhánh `feature/*` riêng biệt được phân công.
> 2. Khi hoàn thành module $\rightarrow$ tạo Pull Request (PR) vào nhánh `develop`.
> 3. Leader (Chan) sẽ thực hiện review source code, kiểm tra logic và bấm Approve PR.
> 4. Sau khi toàn bộ hệ thống hoạt động ổn định trên `develop` $\rightarrow$ merge 1 lần duy nhất vào `main`.
> 5. **`main` chỉ nhận push cuối cùng** – không commit trực tiếp lên `main` trong suốt quá trình phát triển.
>
> 
...
| --- | --- | --- |
| 1 | Giảng viên | Khởi tạo lớp học phần $\rightarrow$ Cấu hình số tín chỉ (Mấy chỉ lý thuyết, mấy chỉ thực hành) $\rightarrow$ Tải lên file điểm thành phần (`diem_thao.csv`). |
...
| 5 | Sinh viên | Đăng nhập hệ thống $\rightarrow$ Xem bảng điểm hiện tại $\rightarrow$ Trình trạng học vụ hiển thị màu xanh (`An toàn`) hoặc nhấp nháy đỏ (`Nguy cơ rớt môn`). |
| 6 | Sinh viên | Kích hoạt lệnh Giả lập $\rightarrow$ Chọn mức điểm chữ mong muốn đạt được (Ví dụ: Loại A). |
...
| 9 | Cố vấn | Truy cập Dashboard $\rightarrow$ Xem danh sách sinh viên thuộc diện cảnh báo học vụ dự báo sớm để tiến hành đôn đốc. |
...
$$T = 20\% \times (\text{ĐTB Thường kỳ}) + 30\% \times (\text{Điểm giữa kỳ}) + 50\% \times (\text{Điểm cuối kỳ})$$


2. **Học phần Thực hành:**

$$T = \frac{\text{Điểm thực hành}_1 + \text{Điểm thực hành}_2 + \dots + \text{Điểm thực hành}_x}{x}$$


3. **Học phần Tích hợp (Cả lý thuyết và thực hành):**

$$T = \frac{(\text{Điểm lý thuyết} \times \text{Số chỉ lý thuyết}) + (\text{Điểm thực hành} \times \text{Số chỉ thực hành})}{\text{Tổng số chỉ học phần}}$$
...
* **Tính bảo trì:**
* Source code Backend FastAPI tổ chức theo kiến trúc phân tầng chuẩn dịch vụ (Router $\rightarrow$ Service $\rightarrow$ Databricks Call Layer), độc lập hoàn toàn với mã nguồn tính toán xử lý dữ liệu lớn (Spark Notebook) trên Databricks.
