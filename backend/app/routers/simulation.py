...
def send_warning_email(
    ...
    details = (
        f"Đã gửi email cảnh báo học vụ khẩn cấp tới sinh viên {req.student_name} ({recipient_email}) "
        f"cho môn {req.ten_mon} ({req.ma_mon}). Lý do: {req.reason}. Nguy cơ rớt môn (RF ML): {req.fail_risk}%."
    )
    ...
    # ─── Gửi thông báo in-app cho Sinh viên khi nhận email cảnh báo ───
    student_user = None
    ...
    if student_user:
        noti = {
            ...
            "message": (
                f"Quản trị viên {current_user.full_name} đã gửi cảnh báo học vụ khẩn cấp môn {req.ten_mon} ({req.ma_mon}) cho bạn. "
                f"Lý do: {req.reason}. Nguy cơ rớt môn (ML): {req.fail_risk}%."
            ),
            "type": "warning",
    ...
    # Giả lập gửi email (in ra console của server FastAPI)
    print(f"\n=========================================================================")
    print(f"SENDING WARNING EMAIL TO: {recipient_email}")
    print(f"SUBJECT: [SmartGPA Warning] Cảnh báo Học vụ Khẩn cấp - Môn {req.ten_mon}")
    print(f"BODY:")
    print(f"Chào {req.student_name},")
    print(f"Chúng tôi gửi cảnh báo học vụ khẩn cấp cho bạn về môn học {req.ten_mon} ({req.ma_mon}).")
    print(f"Lý do cảnh báo: {req.reason}.")
    print(f"Mô hình Random Forest (MLflow Serverless) dự đoán xác suất RỚT MÔN của bạn là: {req.fail_risk}%.")
    print(f"Vui lòng truy cập SmartGPA để lập tức chạy Simulation Engine tính điểm thi cuối kỳ cần đạt mục tiêu qua môn!")
    print(f"Trân trọng,")
    print(f"Quản trị viên: {current_user.full_name}")
    print(f"=========================================================================\n")
    
    return {
        "status": "success",
        "message": f"Email cảnh báo học vụ khẩn cấp đã được gửi thành công đến {recipient_email}!",
...
@router.get(
    ...
    summary="Lấy danh sách thông báo in-app – Chỉ Student",
)
...
@router.post(
    ...
    summary="Đánh dấu đã đọc toàn bộ thông báo in-app – Chỉ Student",
)
def read_all_student_notifications(
    ...
    if user_record and "notifications" in user_record:
        for noti in user_record["notifications"]:
            noti["is_read"] = True
    return {"status": "success", "message": "Đã đánh dấu đọc toàn bộ thông báo thành công."}
